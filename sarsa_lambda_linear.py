...
class SarsaLambdaLinearFA:
    ...
    """
    def __init__(self, function_approximator, n_actions = None, alpha = 0.1, gamma=0.99, lamb=0.9, epsilon=0.05, initial_v=0.0, alpha_bounds=False):
        """
    ...
    def next_move(self, state):
        """Stochastically returns an epsilon-greedy action from the 
            current state. 
        """
    ...
    def update_alpha(self, fa_vals, traces, fa_vals_p, traces_p, terminal):
        ...
        eps_alpha = np.dot(fa_vals, traces) - self.gamma*np.dot(fa_vals_p, traces_p) 

...
# first-order FB - but only the angles, not the velocities. The 
# code below manually hacks the state to remove these and match the 
...
if __name__ == "__main__":
    ...
    for sample in range(0, samples):
        ...
        learner = SarsaLambdaLinearFA(fb, n_actions = n_actions, gamma=gamma, lamb=0.90, epsilon=0.05, alpha=0.001) 
        ...
        for episode in range(0, episodes):
            ...
            # Read env.state directly to avoid additional sin/cos terms.
            s = env.state 
            s = (s - l_state) / d_state 
